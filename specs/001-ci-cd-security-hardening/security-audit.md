...
| SEC-005 | low | outbound-http | `src/comfy.rs`: `upload_image_once` (143-151), `submit_prompt_once` (172-180), `get_history_once` (212-219), `view_once` (266-277) | Response bodies are fully buffered (`.json()`/`.bytes()`) with no size cap; reqwest has no built-in max-response-size guard. | `comfy_url` is operator-trusted, not attacker input, so the realistic trigger is a backend bug (e.g. a corrupted/oversized response), not exploitation — but impact is a possible OOM/crash of the only process on the box. | Fixed on 2026-08-18: all four call sites now buffer through `read_capped`/`json_capped`, which reject a declared `content-length` over the cap up front and abort mid-stream otherwise. Caps are `MAX_JSON_BYTES` (8 MiB) and `MAX_IMAGE_BYTES` (128 MiB). Regression tests: `comfy::tests::read_capped_rejects_a_body_larger_than_the_cap`, `read_capped_returns_a_body_within_the_cap`. | fixed |
...
- **Fixed 2026-08-18** (follow-up pass): SEC-005 (response size caps in
  `src/comfy.rs`)
- **Fixed 2026-08-18** (same follow-up pass): SEC-007 (dedupe atomic-write
  helper into `paths::atomic_write_blocking`)
- **Deferred, still open**: none
